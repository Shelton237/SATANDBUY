...
const { publish } = shared.events.bus;
const { EVENTS: ORDER_EVENTS } = shared.events.orders;

const {
  handleProductQuantity,
} = require("@satandbuy/catalog-domain").lib.productInventory;
const { formatAmountForStripe } = require("../lib/stripe/stripe");
const { releaseDriverSlotsByOrder } =
  require("@satandbuy/delivery-domain").lib;
...
const emitOrderEvent = (eventName, orderDoc, extra = {}) => {
  if (!orderDoc || !eventName) return;
  const payload = {
...
const confirmOrderDelivery = async (req, res) => {
  try {
    ...
    emitOrderEvent(ORDER_EVENTS.ORDER_DELIVERY_CONFIRMED, order, {
